...
| **רב-שפתיות** | `prompts.ts` (חוקה פר-שפה) + מפתוח ה-slug. | לא בונים כלום עכשיו; מתועד המסלול: עמודת `locale @default("he")` + מעבר מ-`slug @unique` ל-`@@unique([locale, slug])` — מיגרציה אחת; החוקה הופכת ממחרוזת למפה פר-locale. שום קוד נוכחי לא מניח עברית מחוץ ל-prompts/UI. |
